...
export const CalendarCell: React.FC<CalendarCellProps>=({date,events,isToday,isCurrentMonth,onClick,openModal,}) => {
  ...
  className={`border p-1 sm:p-2 h-28 sm:h-32 cursor-pointer relative group transition ${!isCurrentMonth ? 
...
>
  ...
  <div className="flex justify-between items-start mb-1">
    ...
    {isToday && (
      <span className="w-5 sm:w-6 h-5 sm:h-6 bg-blue-500 rounded-full text-white text-[9px] sm:text-xs flex items-center justify-center">
        {date.getDate()}
      </span>
    )}
  </div>
  ...
  {/* Events list */}
  <div className="space-y-1 overflow-auto max-h-[4rem] sm:max-h-[5rem] pb-4">
    {events.slice(0, 3).map((e) => (
      <div
        key={e.id}
        className="text-[9px] sm:text-xs px-1 sm:px-2 py-0.5 sm:py-1 rounded truncate text-white shadow-sm hover:opacity-80 transition-opacity"
    ...
    {events.length > 3 && (
      <div className="absolute bottom-1 left-1 sm:left-2 text-[9px] sm:text-xs text-blue-600 hover:underline">
  ...
  <button
    className="absolute top-0.5 sm:top-1 right-0.5 sm:right-1 p-1 text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity rounded-full hover:bg-blue-100"
    onClick={(e) => {
      e.stopPropagation();
      onClick(date);
    }}
